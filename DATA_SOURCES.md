...
| **GDELT Project DOC 2.0** | Fail-soft fallback for location-matched cockpit headlines; source for the **News layer**, ingested locally by `scripts/news-ingest.mjs` into a SQLite database that is never redistributed | [GDELT Terms of Use](https://www.gdeltproject.org/about.html#termsofuse): unrestricted academic/commercial/governmental dataset use, with citation and link required; linked articles retain publisher terms | "GDELT Project" plus each article's linked publisher/domain |
| **Wikimedia EventStreams** (recentchange) | Ambient "pulse" layer of live Wikipedia edit activity worldwide, source for the **Wiki Pulse layer** | CC0 (public-domain event metadata); keyless, no ToS restriction found for this public SSE endpoint | "Wikimedia EventStreams" |
...
| **Minnesota DOT (511mn.org)** | CCTV camera catalog + frames, Twin Cities metro | Public MnDOT traffic camera data; frames via the multi-state [CARS Program](https://www.carsprogram.org/) | "Minnesota Department of Transportation — 511mn.org" |
...
- **Minnesota DOT (511mn.org).** No public API is published for this source; the catalog is fetched by replaying the internal GraphQL endpoint 511mn.org's own site uses (`/api/graphql`), reverse-engineered from its bundled JS since no documentation exists. One bbox-filtered `listCameraViewsQuery` covers the Twin Cities metro core, and — because that query carries no coordinates — one `listMapModalQuery` per kept camera resolves its position by reading the `center=lat,lon` parameter off a server-generated Google Static Maps URL already embedded in the response; no Google Maps key is used or required on this app's end. That per-camera lookup is fetched in bounded batches and capped before it runs (`CCTV_MNDOT_MAX_SOURCES`, default 60, floor 8) specifically to bound its request fan-out, which is the one thing this pack does that Austin/Caltrans/TfL don't. Frames are served from a second, simple, unauthenticated host, `public.carsprogram.org` — the multi-state "CARS Program" (Condition Acquisition and Reporting System) platform. Disable with `CCTV_MNDOT_ENABLED=0`.
...
- **Wikimedia EventStreams.** `/api/wikipulse` relays Wikimedia's public, keyless `recentchange` SSE feed server-side (Node's native `fetch`, no new dependency), filtered to `*.wikipedia.org` edit/new events only, into a capped 300-row in-memory buffer the client polls every 3 s. No API key or quota applies. A blip's globe position is **entirely illustrative** — deterministic per event id but chosen by land-area-weighted random country pick, not derived from the edit's language, wiki, or the editor's location; the in-app card says so. Anonymous editors' IP addresses (Wikimedia's `user` field for unregistered edits) are redacted to "Anonymous editor" server-side, before an event is ever buffered or sent to the client, and redacted again client-side as a second, independent safeguard.
...
Static datasets shipped in the repo for an out-of-the-box experience. **None of the third-party datasets are MIT** — each keeps its own license (see the carve-out in [LICENSE](LICENSE)). Each folder also has its own provenance README.
...
| **Natural Earth country boundaries** (233 countries, 1:10m admin-0) | `natural_earth/countries.json` | **Public domain** | ✅ (no restrictions) | "Made with Natural Earth" (courtesy credit — not legally required) |
...
| **Indian History kingdoms** (49 rulers, Nanda → British Raj: approximate extents, cities, events) | `indian_history/` | **MIT** (original, hand-traced for this project; links to Wikipedia, no content copied) | ✅ | — (Wikipedia is linked, not redistributed) |
